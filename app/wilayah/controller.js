...
async function getKabupaten(req, res, next) {
  ...
  try {
    ...
    return res.json(
      data.filter((kabupaten) => kabupaten.kode_provinsi === kode_provinsi)
    );
  } catch (err) {
    return res.json({
      error: 1,
      message: "Tidak bisa mengambil kabupaten, hubungi administrator",
    });
  }
}
...
async function getKecamatan(req, res, next) {
  ...
  try {
    ...
    if (!kode_induk) return res.json(data);
    res.json(
      data.filter((kecamatan) => kecamatan.kode_kabupaten === kode_kabupaten)
    );
  } catch (err) {
    return res.json({
      error: 1,
...
async function getDesa(req, res, next) {
  const db_desa = path.resolve(__dirname, "./data/vilages.csv");

  try {
    let { kode_induk } = req.query;
    const data = await csv().fromFile(db_desa);
    if (!kode_induk) return res.json(data);
    res.json(data.filter((desa) => desa.kode_kecamatan === kode_kecamatan));
  } catch (err) {
    return res.json({
      error: 1,
